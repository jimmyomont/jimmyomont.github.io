const Sakura = function(selecteur, options) {
    if (typeof selecteur === "undefined") throw new Error("Aucun selecteur specifie. Definissez un element.");
        ...
        colors: [
            {
                ...
                gradientColorStart: "rgba(1, 157, 142, 0.5)",
                gradientColorEnd: "rgba(1, 157, 142, 0.5)",
    ...
    function evenementAvecPrefixe(element, type, rappel) {
        for(let p = 0; p < prefixes.length; p += 1){
            let typeAnim = type;
            if (!prefixes[p]) typeAnim = type.toLowerCase();
    ...
    this.creerPetale = ()=>{
        if (this.el.dataset.sakuraAnimId) setTimeout(()=>{
        ...
        const tempsChute = (document.documentElement.clientHeight * 0.007 + Math.round(Math.random() * 5)) * this.paramètres.fallSpeed;
        // Creer les animations
        const animationsArr = [
            `fall ${tempsChute}s linear 0s 1`,
            `${animationSouffle} ${(tempsChute > 30 ? tempsChute : 30) - 20 + entierAleatoire(0, 20)}s linear 0s infinite`,
        ...
        petale.style.borderRadius = `${entierAleatoire(this.paramètres.maxSize, this.paramètres.maxSize + Math.floor(Math.random() * 10))}px ${entierAleatoire(1, Math.floor(largeur / 4))}px`;
        petale.style.height = `${hauteur}px`;
        petale.style.left = `${Math.random() * document.documentElement.clientWidth - 100}px`;
        ...
        evenementAvecPrefixe(petale, "AnimationEnd", ()=>{
            if (!elementDansFenêtreAffichage(petale)) petale.remove();
        });
    ...
    this.el.setAttribute("data-sakura-anim-id", window.requestAnimationFrame(this.creerPetale));
};
_c = Sakura;
Sakura.prototype.start = function() {
    const animId = this.el.dataset.sakuraAnimId;
    if (!animId) this.el.setAttribute("data-sakura-anim-id", window.requestAnimationFrame(this.creerPetale));
    else throw new Error("Sakura est dej\xe0 en cours d'execution.");
};
Sakura.prototype.stop = function(doux = false) {
    ...
    if (!doux) setTimeout(()=>{
        ...
        while(petales.length > 0)petales[0].parentNode.removeChild(petales[0]);
    }, this.paramètres.delay + 50);
};
var _c;
$RefreshReg$(_c, "Sakura");

//# sourceMappingURL=index.22b6dfd0.js.map
